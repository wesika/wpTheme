...
class wpcuFPN_Front {
	...
	private function container( $is_sidebar_widget=false ) {
		...
		$this->html .= '<div class="wpcufpn_outside" style="' . $style_cont . '">';
		...
		/** Container div **/
		$this->html .= '<div id="wpcufpn_widget_' . $this->widget->ID . '" class="wpcufpn_container ' . $orientation .'" style="' . $style_cont . '">';
		...
		if( 1 == $this->widget->settings['pagination'] || 3 == $this->widget->settings['pagination'] )		// Arrow left
			 $this->html .= '<a href="#" class="slide_left slidebtn">&larr;</a>' .
				'&nbsp;|&nbsp;';
		if( 2 == $this->widget->settings['pagination'] || 3 == $this->widget->settings['pagination'] ) {	// Page numbers
			...
			for( $p=1; $p<=$this->widget->settings['amount_pages']; $p++ ) {
				if( !$first ) $this->html .= '&nbsp;|&nbsp;';
				$first = false;
				$this->html .= '<a href="#" class="pagi_p">' . $p . '</a>';
			}
		}
		if( 3 == $this->widget->settings['pagination'] ) 
			$this->html .= '&nbsp;|&nbsp;';
		if( 1 == $this->widget->settings['pagination'] || 3 == $this->widget->settings['pagination'] )	// Arrow right
			$this->html .= '<a href="#" class="slide_right slidebtn">&rarr;</a>';
		if( $this->widget->settings['pagination'] )
			$this->html .= '</div>';
	...
	private function loop() {
		...
		foreach ( $this->posts as $post ) {
			setup_postdata( $post );
			$this->html .= '<li id="wpcufpn_li_' . $this->widget->ID . '_' . $post->ID . '" style="' . $style . '">';
			$this->frame();
			$this->html .= '</li>';
		}
	}
	...
	private function box_misc( $box_name ) {
		...
		$before = '<div id="wpcufpn_box_' . $box_name . '_' . $this->widget->ID . '_' . $post->ID . '" class="wpcu-front-box ' . $box_name . '" ' . $style . '>';
