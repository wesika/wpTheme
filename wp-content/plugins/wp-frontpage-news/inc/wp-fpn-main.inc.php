	...
	*/
	...
	public function setupCustomPostTypes() {
		$labels = array(
			...
			'all_items' 			=> __( 'All WPFN Blocks', 'wpcufpn' ),
			...
			'parent_item_colon' 	=> '',
			'menu_name' 			=> __( 'WPFN Blocks', 'wpcufpn' )
		);
	...
	function addAdminStylesheets() {
		...
		if( USE_LOCAL_JS_LIBS ) {
			wp_register_style( 'uiStyleSheet', plugins_url( 'css/jquery-ui.css', dirname( __FILE__ ) ) );
		} else {
	...
	*/
		...
		<div style="background:#fff" class="ui-tabs ui-widget ui-widget-content ui-corner-all">
				...
				(function($) {
					$(document).ready(function() {
						...
						*/
						...
						$('.drop_zone_col .wpcu-inner-admin-block ul').sortable({
							connectWith: 'ul.arrow_col',
						...
						$('.drop_zone_col .wpcu-inner-admin-block ul').disableSelection();
			...
			<div id="wpcufpnnavtabs" class="wpcufpntabs">
				<ul>
					...
					<li><a href="#tab-5"><?php _e( 'About', 'wpcufpn' ); ?></a></li>
				</ul>
				...
				<div id="tab-5" class="metabox_tabbed_content">
					<?php $this->displayAboutTab(); ?>
				</div>
			</div>
	...
	private function displayDisplayThemeTab() {
		...
		*/
		...
		if( !isset($settings['theme'] ) ) {
			$theme_dirs = array_keys( $all_themes );
			$settings['theme'] = $theme_dirs[0];
		}
		
		if( isset($all_themes[$settings['theme']]['theme_root']) ) {
			$screenshot_file = $all_themes[$settings['theme']]['theme_root'] . '/' . $settings['theme'] . '/screenshot.png';
		} else {
			$screenshot_file = false;
		}
		//echo 'screenshot file: ' . $screenshot_file . '<br/>';	//Debug
		if( $screenshot_file && file_exists( $screenshot_file ) ) {
			echo '<img alt="preview" src="' . plugins_url( 'themes/' . $settings['theme'] . '/screenshot.png', dirname( __FILE__ ) ) . 
	 ...
	 *
	 */
	private function displayAboutTab() {
	...
	function themeLister() {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				$headers = get_file_data( $theme_root . '/' . $dir . '/style.css', self::$file_headers, 'theme' );
	...
	public function editorFooterScript() {
		...
		$widgets = get_posts( array( 'post_type'=>self::CUSTOM_POST_NEWS_WIDGET_NAME ) );
